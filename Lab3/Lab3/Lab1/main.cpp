...
unsigned int teapot_vao = 0; //vertect array object
int width = 800.0;
int height = 600.0;
GLuint loc1;
GLuint loc2;
...
void display() {
	...
	mat4 persp_proj = perspective(45.0, (float)width / (float)height, 0.1, 100.0); // persepective projection
	mat4 model = rotate_z_deg(identity_mat4(), (timeGetTime() / 20)); // rotates object by 45 degrees
	model=model*rotate_y_deg(identity_mat4(), (timeGetTime()/20));
	model=model*rotate_y_deg(identity_mat4(), (timeGetTime()/20));
	...
	//bottom right persepctive projection and some rotation
	mat4 view1 = translate(identity_mat4(), vec3(0.0, 0.0, -45.0)); //sets camera at -40 on z axis
	mat4 persp_proj1 = perspective(70.0, (float)width/ (float)height, 0.1,100.0); // persepective projection
	...
	// top-left prespective and look at
	//						camera pos				  target pos				  up
	mat4 view2 = look_at(vec3(0.0, -20.0, -60.0), vec3(10.0, 20.0, 10.0), vec3(1.0, 60.0, 90.0));
	mat4 persp_proj2 = perspective(45.0, (float)width / (float)height, 0.1, 100.0); // persepective projection
